...
/* v27 — conversion : bénéfices concrets, réassurance et offres lisibles. */
(() => {
  "use strict";
  if (window.__coachRaphHomeV27) return;
  window.__coachRaphHomeV27 = true;
  ...
  function improveHero(page) {
    const hero = page.querySelector(".coach-home-hero");
    const lead = hero?.querySelector(".coach-home-lead");
    const cta = hero?.querySelector(".coach-home-primary");
    if (!hero || !lead || !cta) return;

    if (!hero.querySelector(".coach-v27-hero-outcome")) {
      const outcome = make("p", "coach-v27-hero-outcome");
      outcome.textContent = "Un programme construit pour que tu progresses, même quand ton quotidien bouge.";
      lead.insertAdjacentElement("afterend", outcome);
    }
    if (!hero.querySelector(".coach-v27-hero-reassurance")) {
      const proof = make("p", "coach-v27-hero-reassurance", "<span>Échange gratuit</span><span>Sans engagement</span><span>Réponse personnalisée</span>");
      cta.insertAdjacentElement("afterend", proof);
    }
  }

  function improveDiscovery(root) {
    const offer = root.querySelector(".coach-v18-offer");
    const card = offer?.querySelector(".coach-v18-offer-card");
    if (!offer || !card || offer.querySelector(".coach-v27-discovery-points")) return;
    const points = make("div", "coach-v27-discovery-points", `
      <span><b>90 €</b><small>prix unique</small></span>
      <span><b>4 semaines</b><small>d’accompagnement</small></span>
      <span><b>Sur mesure</b><small>programme adapté</small></span>
      <span><b>Suivi inclus</b><small>points et ajustements</small></span>
      <span><b>Sans abonnement</b><small>automatique</small></span>`);
    card.insertAdjacentElement("afterend", points);
  }

  function improveTestimonials(root) {
    const labels = [
      [/alexis|consigne|ma[iî]trise/, "Des repères clairs pour progresser"],
      [/miguel|adapter les s[eé]ances|motivant/, "Des séances adaptées à ton niveau"],
      [/olivier|performances|capacités/, "Des progrès ressentis sur la saison"]
    ];
    root.querySelectorAll(".coach-v17-testimonial-card").forEach((card) => {
      if (card.querySelector(".coach-v27-testimonial-result")) return;
      const label = labels.find(([pattern]) => pattern.test(clean(card.textContent)))?.[1];
      const content = card.querySelector(".coach-v17-testimonial-content");
      const stars = content?.querySelector(".coach-v17-testimonial-stars");
      if (!label || !content || !stars) return;
      const result = make("span", "coach-v27-testimonial-result");
      result.textContent = label;
      stars.insertAdjacentElement("afterend", result);
    });
  }

  function improveFinalCta(root) {
    const cta = root.querySelector(".coach-v18-final-cta");
    const content = cta?.querySelector(".notion-callout__content") || cta;
    const link = cta?.querySelector("a[href]");
    if (!cta || !content || !link || cta.dataset.coachV27Final === "1") return;
    const href = link.href;
    cta.dataset.coachV27Final = "1";
    content.innerHTML = `
      <div class="coach-v27-final-copy">
        <span>Sans engagement</span>
        <h2>Le premier pas ne t’engage à rien.</h2>
        <p>Pas besoin d’être déjà prêt : on fait simplement le point sur ta situation et ton objectif.</p>
      </div>
      <a class="coach-v27-final-button" href="${href}">Réserver mon échange gratuit <i aria-hidden="true">→</i></a>`;
  }

  function improvePayment(root) {
    const payment = root.querySelector(".coach-v21-payment");
    if (!payment) return;
    payment.classList.add("coach-v27-payment-secondary");
    const heading = payment.querySelector(".coach-v21-payment-heading p");
    if (heading) heading.textContent = "Choisis ton programme et accède directement au paiement sécurisé.";
    payment.querySelectorAll(".coach-v21-payment-card").forEach((card) => {
      const type = card.dataset.coachPaymentFormula;
      if (!type || card.querySelector(".coach-v27-payment-hint")) return;
      const hint = make("span", "coach-v27-payment-hint");
      hint.textContent = type === "duo" ? "Paiement Duo" : "Paiement unique";
      card.append(hint);
    });
  }
  ...
  function run() {
    const page = document.querySelector("#page-index.coach-home-page");
    const root = page?.querySelector(".notion-root, .notion-page-content");
    if (!page || !root) return;
    improveHero(page);
    improveDiscovery(root);
    improveTestimonials(root);
    improveFinalCta(root);
    improvePayment(root);
  }
...
(() => {
  ...
  if (window.__coachRaphHomeV25) return;
  window.__coachRaphHomeV25 = true;
  ...
  function wrapRole(root, role, className) {
    // The final commercial area is generated independently below. Keeping the
    // Notion "after" blocks in a wrapper makes Super occasionally draw a
    // residual vertical border around the whole lower section.
    if (role === "after") return null;
  ...
  function unwrapAfterShell(root) {
    const shells = [...root.querySelectorAll(":scope > .coach-v18-section.coach-v18-after, :scope > .coach-v18-section[data-coach-v18-role=\"after\"]")];
    shells.forEach((shell) => {
      const fragment = document.createDocumentFragment();
      while (shell.firstChild) fragment.append(shell.firstChild);
      shell.replaceWith(fragment);
    });
  }
  ...
  function consolidateTestimonials(root, testimonials, after) {
    ...
    const finalCta = (after || root).querySelector(".coach-v18-final-cta");
