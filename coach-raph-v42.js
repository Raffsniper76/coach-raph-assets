...
(() => {
  ...
  function setupTestimonials(root) {
    ...
    if (sourceColumns.length < 2) return;

    const currentShell = [...root.querySelectorAll(".coach-v17-testimonials-shell")]
      .find((shell) =>
        shell.dataset.coachTestimonialsGenerated === "true" ||
        (shell.querySelector(".coach-v17-testimonial-card") && !shell.classList.contains("coach-v17-legacy-slider"))
      );

    const existingCount = currentShell?.querySelectorAll(".coach-v17-testimonial-card").length || 0;
    if (currentShell && existingCount === sourceColumns.length) {
      sourceLists.forEach((source) => source.classList.add("coach-v17-testimonials-source"));
      return;
    }

    if (currentShell) {
      const oldTrack = currentShell.querySelector(".coach-v17-testimonials-track");
      const sliderState = oldTrack ? sliders.get(oldTrack) : null;
      if (sliderState?.autoplay) window.clearInterval(sliderState.autoplay);
      if (oldTrack) sliders.delete(oldTrack);
      currentShell.remove();
    }
    ...
    const testimonials = sourceColumns.map((column) => {
      ...
      let context = metaLines[1] || paragraphs[3] || "";
      let tagText = metaLines.slice(2).join(" ") || paragraphs.slice(4).join(" ");

      // Fallback si Super sert encore brièvement l'ancienne signature sur une mutation DOM.
      if (!context && !tagText && author.includes(",")) {
        ...
        if (/joueur de baseball/.test(normalizedRemainder)) context = "Joueur de baseball";
        if (/tests physiques professionnels/.test(normalizedRemainder)) context = "Préparation à des tests physiques professionnels";
        if (/remise en forme|perte de poids/.test(normalizedRemainder)) context = "Remise en forme & perte de poids";
        tagText = remainder;
      }
    ...
    const legacyShells = [...root.querySelectorAll(".coach-v17-testimonials-shell")]
      .filter((shell) => !shell.querySelector(".coach-v17-testimonial-card"));
