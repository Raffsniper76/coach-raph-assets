...
(() => {
  ...
  const runtime = {
    ...
    destroy() {
      ...
      if (window.__coachRaphPublicRefresh === this.refresh) delete window.__coachRaphPublicRefresh;
  ...
  window.__coachRaphPublicUI = runtime;
})();

(() => {
  "use strict";

  window.__coachRaphHomeV172 = "v50";
    ...
    if (!section) {
      ...
      section.innerHTML = `
        <div class="coach-v17-manifesto-main">
          ...
          <p class="coach-v17-kicker">Un cadre qui suit ton rythme.</p>
          <h2 id="coach-v17-manifesto-title">Mon programme s’adapte <br>à ton quotidien.</h2>
          <p class="coach-v17-manifesto-lead">Tu t’entraînes où tu veux, quand ton emploi du temps le permet. Je suis ta progression, je réponds à tes questions et j’ajuste le programme à partir de tes retours.</p>
          <p class="coach-v17-manifesto-note"><strong>Mon objectif :</strong> te transmettre les repères et la méthode nécessaires pour que tu puisses progresser seul, durablement.</p>
        </div>
  ...
  function upgradeFormulas(root) {
    const mapping = [
      ["eec8b2e3ff0449fba655b093f2f1aafe", "sculpture", "Sculpture"],
      ["8b029bcc7a254855b151dfabe893f297", "cardio", "Cardio"],
      ["6f8587bc689948d5b374401a74938339", "ngnp", "No gain, no pause"],
      ["76220e2efe71487b89ec94ec07318cd1", "vitalite", "Vitalité"]
    ];
    mapping.forEach(([id, type, label]) => {
      const card = root.querySelector(`#block-${id}`);
      if (!card) return;
      card.setAttribute("data-coach-v17-formula", type);
      if (!card.querySelector(".coach-v20-formula-photo")) {
        const photo = create("span", "coach-v20-formula-photo");
        photo.dataset.label = label;
  ...
  function setupTestimonials(root) {
    // Important : construire le carrousel une seule fois. Super/Notion continue
    // à muter le DOM après le premier rendu ; on ne supprime/reconstruit jamais
    // un carrousel déjà créé.
    ...
    const sourceLists = [...root.querySelectorAll('.notion-column-list[data-coach-home-section="testimonials"]')]
      .filter((list) => !list.closest(".coach-v17-testimonials-shell"));

    if (!sourceLists.length) return;

    const sourceColumns = sourceLists.flatMap((source) =>
      [...source.querySelectorAll(":scope > .notion-column")]
    );

    // La page contient désormais 5 témoignages. On attend que Notion ait rendu
    // les deux rangées avant de fabriquer le carrousel, au lieu de le reconstruire
    // plus tard quand la seconde rangée apparaît.
    if (sourceColumns.length < 5) return;

    const normalizeMeta = (value = "") => String(value)
      .normalize("NFD")
      .replace(/[\u0300-\u036f]/g, "")
      .replace(/\u00a0/g, " ")
      .replace(/\s+/g, " ")
    ...
    const tagDefinitions = [
      { key: "preparation", label: "Préparation physique", pattern: /preparation physique/ },
      { key: "sculpture", label: "Sculpture sur mesure", pattern: /sculpture sur mesure/ },
      { key: "running", label: "Course à pied", pattern: /course a pied/ },
      { key: "ngnp", label: "No gain, no pause", pattern: /no gain[, ]+no pause/ },
      { key: "vitalite", label: "Vitalité", pattern: /\bvitalite\b/ },
      { key: "cardio", label: "Cardio", pattern: /\bcardio\b/ }
    ];

    const testimonials = sourceColumns.slice(0, 5).map((column) => {
      ...
      const metaLines = (paragraphs[2] || "")
        .split(/\n+/)
        .map((line) => line.trim())
        .filter(Boolean);

      let author = metaLines[0] || paragraphs[2] || "";
      let context = metaLines[1] || "";
      let tagText = metaLines.slice(2).join(" ");

      // Compatibilité avec l'ancienne signature sur une seule ligne.
      if ((!context || !tagText) && author.includes(",")) {
        const legacy = author.replace(/^—\s*/, "").split(",");
        author = legacy.shift()?.trim() || "";
        const remainder = legacy.join(",").trim();
        const normalizedRemainder = normalizeMeta(remainder);

        if (!context && /joueur de baseball/.test(normalizedRemainder)) context = "Joueur de baseball";
        if (!context && /tests physiques professionnels/.test(normalizedRemainder)) context = "Préparation à des tests physiques professionnels";
        if (!context && /remise en forme|perte de poids/.test(normalizedRemainder)) context = "Remise en forme & perte de poids";
        if (!tagText) tagText = remainder;
      }

      author = author.replace(/^—\s*/, "").trim();
      const normalizedTags = normalizeMeta(tagText);
      const tags = tagDefinitions
        .filter((definition) => definition.pattern.test(normalizedTags))
    ...
    // Ordre éditorial uniquement : on garde toute la mécanique v50 intacte.
    // Première vue : un vrai visage + un témoignage anonymisé.
    const testimonialOrder = ["Jonathan", "Alexis", "Aurélia", "Olivier", "Miguel"];
    testimonials.sort((left, right) => {
      const leftIndex = testimonialOrder.indexOf(left.author);
      const rightIndex = testimonialOrder.indexOf(right.author);
      return (leftIndex < 0 ? 99 : leftIndex) - (rightIndex < 0 ? 99 : rightIndex);
    });

    sourceLists.forEach((source) => source.classList.add("coach-v17-testimonials-source"));

    const shell = create("div", "coach-v17-testimonials-shell");
    shell.dataset.coachTestimonialsGenerated = "true";
    ...
    testimonials.forEach((testimonial) => {
      ...
      const meta = create("div", "coach-v17-testimonial-author");
      const name = create("strong", "coach-v17-testimonial-name");
      name.textContent = testimonial.author;
      meta.append(name);

      if (testimonial.context) {
        const context = create("span", "coach-v17-testimonial-context");
        context.textContent = testimonial.context;
        meta.append(context);
      }

      if (testimonial.tags.length) {
        const tags = create("div", "coach-v17-testimonial-tags");
        testimonial.tags.forEach((testimonialTag) => {
          const tag = create("span", "coach-v17-testimonial-tag");
          tag.dataset.coachTestimonialTag = testimonialTag.key;
          tag.textContent = testimonialTag.label;
          tags.append(tag);
        });
        meta.append(tags);
      }

      content.append(stars, title, quote, meta);
      article.append(avatar, content);
      track.append(article);
    });

    const insertionTarget = legacyShells[0] || sourceLists[0];
    ...
    const render = () => {
      /* Pas de scrollTo/scrollLeft ici : dans l’aperçu Super, le déplacement
         programmatique du scroller peut provoquer un recalcul de la page et
         masquer les sections centrales. On fait donc tourner les cartes avec
         la propriété flex `order`, sans déplacer aucun conteneur. */
      cards.forEach((card, cardIndex) => {
        card.style.order = String((cardIndex - index + cards.length) % cards.length);
      });
    ...
    controls.addEventListener("click", (event) => {
      ...
      move(Number(button.dataset.dir));
    });
    render();
    ...
    if (!reducedMotion) {
      ...
      window.__coachRaphPublicUI?.intervals.push(autoplay);
    }
  }
  ...
  window.__coachRaphHomeCoreRefresh = upgradeHome;
  upgradeHome();
})();
(() => {
  "use strict";

  window.__coachRaphHomeV361 = "v50";
  ...
  function orderSections(root, shells) {
    ...
    [shells.formulas, shells.discovery, shells.details, shells.coach, shells.testimonials, shells.after]
      .filter(Boolean)
      .forEach((section) => {
        if (section.closest(".coach-v35-commercial-flow")) return;
  ...
  function upgradeDetails(section) {
    ...
    if (existing) {
      if (!existing.querySelector(".coach-v30-route")) {
        existing.insertAdjacentHTML("afterbegin", '<span class="coach-v30-route" aria-hidden="true"></span>');
      }
    ...
    timeline.innerHTML = '<span class="coach-v30-route" aria-hidden="true"></span>';
    items.forEach((item, index) => {
  ...
  function readGuidanceFromNotion(source) {
    const content = source.querySelector(".notion-callout__content") || source;
    const links = [...content.querySelectorAll("a[href]")];
    const normalizeText = (value) => String(value || "").replace(/\u00a0/g, " ").replace(/\s+/g, " ").trim();
    const link =
      links.find((node) => /calendly\.com/i.test(node.href || "") && normalizeText(node.textContent)) ||
      links.find((node) => normalizeText(node.textContent)) ||
      links.find((node) => /calendly\.com/i.test(node.href || "")) ||
      links.at(-1) ||
      null;
    const titleNode = content.querySelector("strong");
    const title = normalizeText(titleNode?.textContent);
    const button = normalizeText(link?.textContent || link?.getAttribute("aria-label") || link?.getAttribute("title"));

    const clone = content.cloneNode(true);
    clone.querySelectorAll("a, strong, button, svg, img, .notion-callout__icon").forEach((node) => node.remove());
    ...
    return {
      title: title || "Tu hésites entre plusieurs programmes ?",
      description: description || "Je t’aide à identifier celui qui correspond le mieux à ton objectif, ton niveau et tes contraintes.",
      button: button || "Choisir un créneau avec Raphaël",
      href: link?.href || "#"
    };
  }
  ...
  function upgradeGuidance(root) {
    ...
    const copy = readGuidanceFromNotion(source);
    ...
    if (!panel) {
      ...
      panel.innerHTML = `
        ...
        <a href="#"><span class="coach-v182-guidance-label"></span> <span aria-hidden="true">→</span></a>`;
      source.insertAdjacentElement("afterend", panel);
    }

    const heading = panel.querySelector("strong");
    const paragraph = panel.querySelector("p");
    const panelLink = panel.querySelector("a[href]");
    const label = panel.querySelector(".coach-v182-guidance-label");

    if (heading && heading.textContent !== copy.title) heading.textContent = copy.title;
    if (paragraph && paragraph.textContent !== copy.description) paragraph.textContent = copy.description;
    if (label && label.textContent !== copy.button) label.textContent = copy.button;
    if (panelLink && panelLink.href !== copy.href) panelLink.href = copy.href;
  ...
  function buildAfterCommercial(root, anchor) {
    ...
    if (panel && duoPanel) {
      if (panel.closest(".coach-v35-commercial-flow") && duoPanel.closest(".coach-v35-commercial-flow")) {
        return panel;
      }
      /* Les blocs générés doivent rester au premier niveau de la section.
         Sinon un changement de structure Notion peut les enfermer dans la
         source qui est ensuite masquée. */
      anchor.insertAdjacentElement("beforebegin", panel);
      panel.insertAdjacentElement("afterend", duoPanel);
      return panel;
    }
    panel?.remove();
    duoPanel?.remove();
    ...
    const newDuo = create("aside", "coach-v184-duo-card coach-v22-duo-card");
    newDuo.id = "coach-duo-v184";
    newDuo.dataset.coachGenerated = "duo-offer";
    newDuo.innerHTML = `
      <div class="coach-v22-duo-visual" role="img" aria-label="Deux personnes qui courent ensemble">
        <span>Progresser à deux</span>
      </div>
      <div class="coach-v22-duo-content">
        <span class="coach-v22-duo-eyebrow">Une autre façon de commencer</span>
        <h3>La formule Duo</h3>
        <p class="coach-v22-duo-lead">Un accompagnement personnalisé pour avancer ensemble, chacun à son niveau.</p>
        <div class="coach-v22-duo-price"><strong>150&nbsp;€</strong><span>pour deux personnes pendant 4 semaines<b>soit 75&nbsp;€ par personne</b></span></div>
        <ul><li>Deux bilans individuels</li><li>Un programme adapté à chacun</li><li>Des défis communs</li><li>Des conseils pour s’entraîner ensemble</li></ul>
        ${duoLink ? `<a href="${duoLink.href}" aria-label="Découvrir la formule Duo">Découvrir la formule Duo <span aria-hidden="true">→</span></a>` : ""}
      </div>`;
  ...
  function buildPayment(root, source) {
    if (!source || root.querySelector(".coach-v21-payment")) return;
    const paymentLinks = [...source.querySelectorAll("a[href]")];
    const findPaymentLink = (pattern) => paymentLinks.find((link) => pattern.test(normalize(link.textContent)));
    const duoSource = [...root.querySelectorAll("details, .notion-toggle")]
      .find((node) => /commencer a deux|formule duo/.test(normalize(node.textContent)));
    const duoLink = findPaymentLink(/\bduo\b/) || duoSource?.querySelector("a[href]");
    const formulas = [
      ["sculpture", "Sculpture sur mesure", "Renforcement et transformation", "90&nbsp;€", findPaymentLink(/sculpture sur mesure/)],
      ["cardio", "Cardio", "Souffle et endurance", "90&nbsp;€", findPaymentLink(/^cardio\b|\bcardio\s*[—-]/)],
      ["ngnp", "No gain, no pause", "Force et performance", "90&nbsp;€", findPaymentLink(/no gain.*no pause/)],
      ["vitalite", "Vitalité", "Énergie et bien-être", "90&nbsp;€", findPaymentLink(/vitalite/)],
      ["duo", "Formule Duo", "Avancer ensemble, chacun à son niveau", "150&nbsp;€", duoLink]
    ];
    const payment = create("section", "coach-v21-payment");
    payment.dataset.coachGenerated = "secure-payment";
    payment.setAttribute("aria-labelledby", "coach-v21-payment-title");
    payment.innerHTML = `
      <header class="coach-v21-payment-heading">
        <span class="coach-v21-payment-lock" aria-hidden="true"><svg viewBox="0 0 32 32"><rect x="7" y="14" width="18" height="13" rx="3"/><path d="M11 14V10a5 5 0 0 1 10 0v4M16 19v3"/></svg></span>
        <div><small>Paiement direct</small><h2 id="coach-v21-payment-title">Tu sais déjà ce qu’il te faut&nbsp;?</h2><p>Accède directement à la formule de ton choix et au paiement sécurisé.</p></div>
        <span class="coach-v21-stripe-badge"><b aria-hidden="true">✓</b> Sécurisé par Stripe</span>
      </header>
      <div class="coach-v21-payment-grid">${formulas.map(([type,label,description,price,link]) => link ? `
        <a class="coach-v21-payment-card" data-coach-payment-formula="${type}" href="${link.href}" aria-label="Choisir ${label}">
          <span class="coach-v21-payment-photo" aria-hidden="true"></span>
          <span class="coach-v21-payment-copy"><small>${type === "duo" ? "À deux" : "Formule Découverte"}</small><strong>${label}</strong><em>${description}</em><b>${price}</b></span>
          <i aria-hidden="true">→</i>
        </a>` : "").join("")}</div>
      <footer><span aria-hidden="true">✓</span> Paiement unique et sécurisé. Aucun abonnement automatique.</footer>`;
    source.insertAdjacentElement("beforebegin", payment);
    hideSource(source, "coach-v21-payment-source");
  }
  ...
  function upgradeAfterSafely(root, section) {
    ...
    root.querySelectorAll(".coach-v184-payment").forEach((node) => node.remove());
    ...
    const planSource = [...root.querySelectorAll(".notion-column-list")].find((list) => {
      const text = normalize(list.textContent);
      return [/continuer seul/, /suivi 1 mois/, /suivi 3 mois/, /suivi 6 mois/]
    ...
    const sourceAnchor = afterHeading?.closest("[id^='block-']") || afterHeading || planSource;
    const anchor = directChildContaining(scope, sourceAnchor) || sourceAnchor;
    if (anchor) {
      buildAfterCommercial(root, anchor);
      const headingTarget = afterHeading?.closest("[id^='block-']") || afterHeading;
      hideSource(headingTarget, "coach-v184-after-heading-source");
      const subtitle = headingTarget?.nextElementSibling;
    ...
    [...root.querySelectorAll("details, .notion-toggle")].forEach((toggle) => {
      ...
      if ((/tu sais quelle formule te convient/.test(text) || /tu sais deja ce qu.il te faut/.test(text) || /deja echange avec coach raph/.test(text)) && /paiement securise/.test(text)) buildPayment(root, toggle);
    });
    ...
    const contact = [...scope.querySelectorAll("[id^='block-'], p, div")]
      .filter((node) => {
        const text = normalize(node.textContent);
        return /une question avant de reserver/.test(text) && text.length < 220;
      })
      .sort((a, b) => normalize(a.textContent).length - normalize(b.textContent).length)[0];
    contact?.classList.add("coach-v18-contact");
  }
  ...
  function upgradeMotivationStory(root) {
    ...
    const heading = [...root.querySelectorAll("h1,h2,h3,.notion-heading")]
      .find((node) => /envie est la.*plus dur.*(?:durer|continuer)/.test(normalize(node.textContent)));
    if (!heading) return;
    ...
    story.innerHTML = `
      <header class="coach-v192-motivation-heading">
        ...
        <h2 id="coach-v192-motivation-title">L’envie est là.<br>Le plus dur, c’est de continuer.</h2>
  ...
  /* Super peut laisser une bordure sur une enveloppe Notion qui regroupe
     plusieurs blocs commerciaux. Cette enveloppe n'existe qu'apres les
     transformations ci-dessous : on la detecte donc a partir des blocs finaux. */
  function markCommercialWrappers(section, root) {
    if (!section) return;

    const landmarks = [
      section.querySelector(".coach-v184-after-commercial"),
      section.querySelector(".coach-v22-duo-card"),
      section.querySelector(".coach-v18-testimonials-shell-section"),
      section.querySelector(".coach-v18-final-cta"),
      section.querySelector(".coach-v21-payment"),
      section.querySelector(".coach-v18-contact")
    ].filter(Boolean);

    const candidates = [section, ...section.querySelectorAll("section, aside, blockquote, .notion-quote, .notion-callout, .notion-column-list, [id^='block-']")];
    candidates.forEach((node) => {
      const contained = landmarks.filter((landmark) => node.contains(landmark)).length;
      if (node === section || contained >= 3) {
        node.classList.add("coach-v34-commercial-wrap");
  ...
  function buildLegalFooter(root) {
    ...
    /* Le footer est désormais idempotent : on le crée une seule fois, puis on
       met uniquement ses URL à jour si Super finit de rendre les liens Notion. */
    ...
    if (!footer) {
      ...
      const title = create("span", "coach-v36-legal-title");
      title.textContent = "Informations légales";
      const list = create("ul", "coach-v36-legal-list");
      definitions.forEach(({ label, fallbackHref }) => {
    ...
    const sourceLinks = [...root.querySelectorAll("a[href]")]
      .filter((link) => !link.closest(".coach-v36-legal"));
    const footerLinks = [...footer.querySelectorAll("a[href]")];
    definitions.forEach(({ pattern, label, fallbackHref }, index) => {
      const source = sourceLinks.find((link) => pattern.test(normalize(link.textContent)));
      const target = footerLinks[index];
      if (!target) return;
      const nextHref = source?.href || fallbackHref;
      if (target.href !== nextHref) target.href = nextHref;
  ...
  /* Super place tous les blocs suivant « Et après les 4 semaines ? » dans une
     même enveloppe Notion. On sort les composants finis de cette enveloppe,
     puis on masque uniquement la structure source. Le parcours commercial
     reste ainsi stable, sans bordure parasite et sans perdre la carte Duo. */
  function finalizeCommercialFlow(root, shells) {
    const after = shells.after;
    if (!after) return;

    let flow = root.querySelector(":scope > .coach-v35-commercial-flow");
    if (!flow) {
      flow = create("section", "coach-v35-commercial-flow");
      flow.dataset.coachGenerated = "commercial-flow";
      flow.setAttribute("aria-label", "Offres, témoignages et réservation");
    }

    const coachSection = shells.coach;
    if (!flow.parentElement) {
      if (coachSection) coachSection.insertAdjacentElement("afterend", flow);
      else root.append(flow);
    } else if (coachSection && coachSection.nextElementSibling !== flow) {
      coachSection.insertAdjacentElement("afterend", flow);
    }

    const legalFooter = buildLegalFooter(root);
    const orderedNodes = [
      root.querySelector("#coach-after-v184"),
      root.querySelector("#coach-duo-v184"),
      shells.testimonials,
      after.querySelector(".coach-v18-final-cta"),
      root.querySelector(".coach-v21-payment"),
      after.querySelector(".coach-v18-contact"),
      legalFooter
    ].filter(Boolean);

    orderedNodes.forEach((node) => {
      if (node === flow) return;
      node.classList.remove("coach-v33-commercial-wrap", "coach-v34-commercial-wrap");
      node.style.removeProperty("border-left");
      node.style.removeProperty("border-inline-start");
      node.style.removeProperty("outline");
      if (node.parentElement !== flow) flow.append(node);
    });

    /* Certains thèmes Notion affichent le marqueur Markdown « ### » comme un
       bloc. Il n'a aucune valeur dans le pied de page final. */
    [...after.querySelectorAll("p, h1, h2, h3, .notion-heading")].forEach((node) => {
      if (normalize(node.textContent) === "###") hideSource(node, "coach-v35-markdown-source");
    });

    after.classList.add("coach-v35-source-shell");
  ...
  function upgradeHome() {
    ...
    markCommercialWrappers(shells.after, root);
    finalizeCommercialFlow(root, shells);
  ...
  function upgradeExerciseCards() {
    const page = document.querySelector("#page-index");
    if (!page) return;

    const pageTitle = normalize(document.title || page.querySelector("h1")?.textContent || "");
    const pageText = normalize(page.querySelector("h1")?.textContent || "");
    if (!/seance/.test(`${pageTitle} ${pageText}`)) return;

    [...page.querySelectorAll(".notion-toggle, details")].forEach((toggle) => {
      const summary = toggle.querySelector(".notion-toggle__summary, summary");
      const summaryText = normalize(summary?.textContent || "");
      if (!/technique.*reperes.*squat/.test(summaryText)) return;
      if (toggle.dataset.coachExercisePremium === "true") return;

      const content = toggle.querySelector(".notion-toggle__content") || [...toggle.children].find((child) => child !== summary);
      if (!summary || !content) return;

      toggle.dataset.coachExercisePremium = "true";
      toggle.classList.add("coach-exercise-premium", "coach-exercise-squat");
      summary.classList.add("coach-exercise-premium-summary");
      summary.setAttribute("aria-label", "Afficher ou masquer la fiche technique du squat");

      const children = [...content.children];
      if (!children.length) return;

      const textOf = (node) => normalize(node?.textContent || "");
      const labels = ["objectif", "points cles", "erreurs a eviter", "variantes"];
      const titleNode = children.find((node) => /^squat sur chaise$/.test(textOf(node)));
      const metaNode = titleNode?.nextElementSibling && /renforcement|debutant|chaise/.test(textOf(titleNode.nextElementSibling))
        ? titleNode.nextElementSibling
        : null;

      const header = document.createElement("div");
      header.className = "coach-exercise-premium-header";
      header.dataset.coachGenerated = "exercise-header";
      header.innerHTML = `
        <span class="coach-exercise-premium-icon" aria-hidden="true">
          <svg viewBox="0 0 32 32" fill="none"><path d="M7 8v16M25 8v16M4 12h6M22 12h6M10 16h12M10 20h12"/><circle cx="16" cy="8" r="3"/><path d="M12 12l4 5 4-5"/></svg>
        </span>
        <div>
          <small>Fiche mouvement</small>
          <strong>${titleNode ? titleNode.textContent.trim() : "Squat sur chaise"}</strong>
          <span>${metaNode ? metaNode.textContent.trim() : "Renforcement · Débutant · Chaise stable"}</span>
        </div>`;
      content.prepend(header);
      titleNode?.classList.add("coach-exercise-source-hidden");
      metaNode?.classList.add("coach-exercise-source-hidden");

      const sourceChildren = [...content.children].filter((node) => node !== header && !node.classList.contains("coach-exercise-source-hidden"));
      const markers = sourceChildren
        .map((node, index) => ({ node, index, text: textOf(node) }))
        .filter((entry) => labels.some((label) => entry.text === label || entry.text.startsWith(`${label} `)));

      markers.forEach((marker, markerIndex) => {
        const end = markers[markerIndex + 1]?.index ?? sourceChildren.length;
        const groupNodes = sourceChildren.slice(marker.index, end).filter((node) => node.isConnected && node.parentElement === content);
        if (!groupNodes.length) return;

        const section = document.createElement("section");
        const key = marker.text.startsWith("points cles") ? "points"
          : marker.text.startsWith("erreurs a eviter") ? "errors"
          : marker.text.startsWith("variantes") ? "variants"
          : "objective";
        section.className = `coach-exercise-premium-section coach-exercise-${key}`;
        section.dataset.coachGenerated = `exercise-${key}`;
        content.insertBefore(section, groupNodes[0]);
        groupNodes.forEach((node) => section.append(node));
        marker.node.classList.add("coach-exercise-premium-label");
      });

      const objective = content.querySelector(".coach-exercise-objective");
      const points = content.querySelector(".coach-exercise-points");
      const errors = content.querySelector(".coach-exercise-errors");
      const variants = content.querySelector(".coach-exercise-variants");
      if (points && errors && !content.querySelector(".coach-exercise-premium-grid")) {
        const grid = document.createElement("div");
        grid.className = "coach-exercise-premium-grid";
        grid.dataset.coachGenerated = "exercise-grid";
        content.insertBefore(grid, points);
        grid.append(points, errors);
      }

      const quotes = [...content.querySelectorAll("blockquote, .notion-quote")];
      quotes.forEach((quote) => {
        if (/astuce coach raph/.test(textOf(quote))) quote.classList.add("coach-exercise-premium-tip");
      });

      objective?.classList.add("is-ready");
      variants?.classList.add("is-ready");
    });
  }

  function observeExternalChanges() {
    const runtime = window.__coachRaphPublicUI;
  ...
  function runCleanPass() {
    ...
    runtime.timer = window.setTimeout(() => {
      if (runtime.upgrading) return;
      runtime.upgrading = true;

      /* Même principe que la v38 stable : pendant nos propres déplacements,
         l'observer public est coupé. Les mutations que nous créons ne peuvent
         donc pas relancer la transformation et perturber hover, clics ou
         sélection de texte. */
      runtime.observer?.disconnect();
      try {
        window.__coachRaphHomeCoreRefresh?.();
        upgradeHome();
        upgradeExerciseCards();
      } finally {
    ...
    runtime.observer = new MutationObserver((mutations) => {
      ...
      const externalChange = mutations.some((mutation) =>
        [...mutation.addedNodes, ...mutation.removedNodes].some((node) => {
          ...
          return !node.closest?.("[data-coach-generated], .coach-v184-payment");
        })
      );

      if (externalChange) runCleanPass();
    });
    runtime.refresh = runCleanPass;
    runtime.loadHandler = runCleanPass;
  }

  window.__coachRaphPublicRefresh = runCleanPass;
  observeExternalChanges();
  runCleanPass();
  window.addEventListener("load", runCleanPass, { once: true });
})();


/* ============================================================
   v52 — Extension validée : Le vrai défi + CTA final
   ============================================================ */
...
(() => {
  ...
  function ensureValidatedCtaContent() {
    document.querySelectorAll("#page-index.coach-home-page .notion-callout").forEach((callout) => {
      const link = callout.querySelector('a[href*="calendly"]');
      if (!link) return;

      const text = (callout.textContent || "").replace(/\s+/g, " ").trim().toLowerCase();
      const isFinalCta = text.includes("parlons de ton objectif") || text.includes("le premier pas ne t’engage à rien") || text.includes("le premier pas ne t'engage à rien");
      if (!isFinalCta) return;

      callout.classList.add("coach-v18-final-cta");
      const paragraphs = [...callout.querySelectorAll(".notion-callout__content > p")];
      if (paragraphs[0]) paragraphs[0].textContent = "Parlons de ton objectif.";
      if (paragraphs[1]) paragraphs[1].textContent = "15 minutes pour faire le point sur ton niveau, tes contraintes et ce que tu veux vraiment améliorer. Je te dirai simplement si mon accompagnement est adapté — et par où commencer.";
    });
  }

  function applyChallengeStory() {
    ensureValidatedCtaContent();
...
/* ==========================================================
   V55 — FORMULA MODAL POLISH
   Uses the existing Notion formula toggle as the content source.
   No observer, timer or scrolling behaviour is added here.
   ========================================================== */
(()=>{
  "use strict";
  ...
  const identify=(text)=>{
    ...
    if(value.includes("sculpture sur mesure")) return "sculpture";
    if(value.includes("no gain") || value.includes("crossfit")) return "ngnp";
    if(value.includes("vitalite")) return "vitalite";
    if(value.includes("cardio") || value.includes("course")) return "cardio";
    return "default";
  };

  const splitSummary=(text)=>{
    const value=clean(text);
    const parts=value.split(/\s+[—–-]\s+/);
    return {
      title:parts.shift() || "Programme",
      subtitle:parts.join(" — ")
    };
  };

  const directHeadingIndex=(children,pattern)=>children.findIndex((node)=>pattern.test(norm(node.textContent)));

  const structurePanels=(content)=>{
    if(!content || content.querySelector(":scope > .coach-popup-v55-panels")) return;
    const children=[...content.children];
    const idealIndex=directHeadingIndex(children,/^ideal si tu veux\b/);
    const receiveIndex=directHeadingIndex(children,/^tu recois\b/);
    if(idealIndex<0 || receiveIndex<=idealIndex) return;

    const prefix=children.slice(0,idealIndex);
    const firstNodes=children.slice(idealIndex,receiveIndex);
    const secondNodes=children.slice(receiveIndex);
    if(!firstNodes.length || !secondNodes.length) return;

    const panels=document.createElement("div");
    panels.className="coach-popup-v55-panels";
    const first=document.createElement("section");
    first.className="coach-popup-v55-panel";
    const second=document.createElement("section");
    second.className="coach-popup-v55-panel";

    firstNodes[0]?.classList.add("coach-popup-v55-panel-title");
    secondNodes[0]?.classList.add("coach-popup-v55-panel-title");
    first.append(...firstNodes);
    second.append(...secondNodes);
    panels.append(first,second);
    content.replaceChildren(...prefix,panels);
  };

  const enhance=(summary)=>{
    const popup=document.querySelector(".coach-popup.show");
    const box=popup?.querySelector(".coach-popup-box");
    const content=popup?.querySelector(".coach-popup-content");
    if(!box || !content) return;

    const info=splitSummary(summary?.textContent || "");
    box.classList.add("coach-popup-v55");
    box.dataset.formula=identify(summary?.textContent || "");

    let header=box.querySelector(":scope > .coach-popup-v55-header");
    if(!header){
      header=document.createElement("header");
      header.className="coach-popup-v55-header";
      header.innerHTML='<span class="coach-popup-v55-kicker">Programme</span><h2 class="coach-popup-v55-title"></h2><p class="coach-popup-v55-subtitle"></p>';
      box.insertBefore(header,content);
    }
    header.querySelector(".coach-popup-v55-title").textContent=info.title;
    const subtitle=header.querySelector(".coach-popup-v55-subtitle");
    subtitle.textContent=info.subtitle;
    subtitle.hidden=!info.subtitle;

    const close=box.querySelector(".coach-popup-close");
    if(close && close.textContent!=="×") close.textContent="×";

    structurePanels(content);
  };

  const schedule=(summary)=>requestAnimationFrame(()=>enhance(summary));

  document.addEventListener("click",(event)=>{
    const summary=event.target.closest?.(".coach-formula-summary");
    if(summary) schedule(summary);
  },true);
