/* Coach Raph — v45 CLEAN
   ...
   - La page publique utilise des passes de bootstrap bornées, sans observateur propre.
...
(() => {
  ...
  function isHomeSourceReady() {
    const page = document.querySelector("#page-index.coach-home-page");
    const root = page?.querySelector(".notion-root");
    if (!page || !root) return false;

    const requiredRoles = ["method", "discovery", "formulas", "coach", "testimonials", "details", "after"];
    const hasAllSections = requiredRoles.every((role) =>
      root.querySelector(`[data-coach-home-heading="${role}"]`)
    );
    if (!hasAllSections) return false;

    // The testimonials are the last source block that Notion commonly hydrates
    // in the Super preview. Do not build the public layout until all five exist.
    const sourceLists = [...root.querySelectorAll('.notion-column-list[data-coach-home-section="testimonials"]')]
      .filter((list) => !list.closest(".coach-v17-testimonials-shell"));
    const testimonialColumns = sourceLists.flatMap((list) =>
      [...list.querySelectorAll(":scope > .notion-column")]
    );

    return testimonialColumns.length >= 5 || root.querySelectorAll(".coach-v17-testimonial-card").length >= 5;
  }

  let bootstrapFinished = false;
  let bootstrapTimer = 0;
  const bootstrapStartedAt = Date.now();
  const BOOTSTRAP_MAX_MS = 30000;

  function bootstrapHome() {
    if (bootstrapFinished) return;
    window.clearTimeout(bootstrapTimer);

    // The base layer may run safely while Notion is hydrating: it only marks
    // sections and creates the stable hero/navigation shell.
    window.__coachRaphHomeCoreRefresh?.();

    if (isHomeSourceReady()) {
      bootstrapFinished = true;
      window.requestAnimationFrame(() => upgradeHome());
      return;
    }

    if (Date.now() - bootstrapStartedAt >= BOOTSTRAP_MAX_MS) {
      // Last best-effort pass: never poll forever if Super changes its markup.
      bootstrapFinished = true;
      window.requestAnimationFrame(() => upgradeHome());
      return;
    }

    bootstrapTimer = window.setTimeout(bootstrapHome, 350);
  }

  bootstrapHome();
  window.addEventListener("load", bootstrapHome, { once: true });
})();
