(() => {
  ...
  if (window.__coachRaphHomeV183) return;
  window.__coachRaphHomeV183 = true;
  ...
  function upgradeDetails(section) {
    if (!section) return;
    const existingTimeline = section.querySelector(".coach-v18-timeline");
    if (existingTimeline) {
      existingTimeline.querySelectorAll(".coach-v18-timeline-step").forEach((step, index) => {
        const paragraph = step.querySelector("p");
        if (paragraph && (!normalize(paragraph.textContent) || /une etape personnalisee/.test(normalize(paragraph.textContent)))) {
          paragraph.textContent = detailDescriptions[index] || "";
        }
      });
      return;
    }
  ...
  function buildAfterCommercial(section, sourceRoot = section) {
    let panel = section.querySelector(".coach-v182-after-commercial");
    if (panel) return panel;
    const duoSource = [...sourceRoot.querySelectorAll("details, .notion-toggle")]
  ...
  function findTextNode(root, pattern, selector = "h1,h2,h3,p,.notion-heading,.notion-toggle,.notion-callout") {
    return [...root.querySelectorAll(selector)].find((node) => pattern.test(normalize(node.textContent))) || null;
  }

  function hideLegacyTestimonials(root) {
    const testimonialShells = [...root.querySelectorAll(".coach-v17-testimonials-shell")];
    const preferred = testimonialShells.find((shell) => shell.querySelector(".coach-v17-testimonial-card") && !shell.classList.contains("coach-v17-legacy-slider"));
    testimonialShells.forEach((shell) => {
      if (shell !== preferred) hideSource(shell, "coach-v183-testimonial-source");
    });

    root.querySelectorAll('[data-coach-home-section="testimonials"]').forEach((node) => {
      const isHeading = node.matches('[data-coach-home-heading="testimonials"]') || Boolean(node.querySelector?.('[data-coach-home-heading="testimonials"]'));
      if (!isHeading && !node.contains(preferred)) hideSource(node, "coach-v183-testimonial-source");
    });

    root.querySelectorAll(".notion-column-list").forEach((list) => {
      const text = normalize(list.textContent);
      if (/avec coach raph.*plus qu.a|super motivant.*competent|effets se sont fait ressentir/.test(text)) {
        hideSource(list, "coach-v183-testimonial-source");
      }
    });
  }

  function upgradeAfterGlobally(root) {
    const testimonialSection = root.querySelector('.coach-v18-section[data-coach-v18-role="testimonials"]') || root.querySelector('[data-coach-home-heading="testimonials"]')?.closest('[data-coach-home-section="testimonials"]');
    const afterHeading = root.querySelector('[data-coach-home-heading="after"]') || findTextNode(root, /^et apres les 4 semaines/);
    if (!afterHeading) return;

    let commercial = root.querySelector(".coach-v182-after-commercial");
    if (!commercial) {
      const host = create("section", "coach-v18-section coach-v18-after coach-v183-after-host");
      commercial = buildAfterCommercial(host, root);
      if (testimonialSection) testimonialSection.insertAdjacentElement("beforebegin", host);
      else afterHeading.closest("[id^='block-']")?.insertAdjacentElement("beforebegin", host);
    }

    const afterBlock = afterHeading.closest("[id^='block-']") || afterHeading;
    hideSource(afterBlock, "coach-v183-after-source");

    root.querySelectorAll(".notion-column-list").forEach((list) => {
      const text = normalize(list.textContent);
      if (/continuer seul.*0.*suivi 1 mois.*50|suivi 3 mois.*135.*suivi 6 mois.*240/.test(text)) {
        hideSource(list, "coach-v183-after-source");
      }
    });
    root.querySelectorAll("details,.notion-toggle").forEach((toggle) => {
      if (/commencer a deux|formule duo/.test(normalize(toggle.textContent))) hideSource(toggle, "coach-v183-after-source");
    });
  }

  function upgradePaymentGlobally(root) {
    const source = [...root.querySelectorAll("details,.notion-toggle")]
      .find((node) => /deja echange avec coach raph|acceder au paiement securise/.test(normalize(node.textContent)));
    if (!source) return;
    let payment = root.querySelector(".coach-v182-payment");
    if (!payment) {
      buildPayment(root, source);
      payment = root.querySelector(".coach-v182-payment");
    }
    const finalCta = root.querySelector(".coach-v18-final-cta");
    if (payment && finalCta && finalCta.nextElementSibling !== payment) finalCta.insertAdjacentElement("afterend", payment);
    hideSource(source, "coach-v183-payment-source");
  }
  ...
  function upgradeHome() {
    ...
    upgradeAfterGlobally(root);
    hideLegacyTestimonials(root);
    upgradePaymentGlobally(root);
