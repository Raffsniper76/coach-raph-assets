...
(() => {
  ...
  function runCleanPass() {
    // The base layer marks the Notion sections. The public layer then enhances
    // them. No permanent observer is needed here: a short bounded bootstrap is
    // enough for Super/Notion's asynchronous first render.
    window.__coachRaphHomeCoreRefresh?.();
    window.requestAnimationFrame(() => upgradeHome());
  }

  // Super/Notion may take noticeably longer after a hard refresh because the
  // page source, images and custom assets all start cold. Keep the same clean
  // v45 behaviour, but allow a few extra bounded passes instead of stopping
  // after seven seconds. No permanent polling or extra observer is introduced.
  const bootstrapDelays = [0, 250, 700, 1400, 2600, 4500, 7000, 10000, 14000, 20000];
  bootstrapDelays.forEach((delay) => window.setTimeout(runCleanPass, delay));
