...
(() => {
  ...
  function buildPayment(root, source) {
    ...
    const formulas = [
      ["sculpture", "Sculpture sur mesure", "Renforcement et transformation"],
      ["cardio", "Cardio", "Souffle et endurance"],
      ["ngnp", "No gain, no pause", "Force et performance"],
      ["vitalite", "Vitalité", "Énergie et bien-être"]
    ];
    ...
    payment.innerHTML = `
      ...
      <div class="coach-v21-payment-grid">${formulas.map(([type,label,description],index) => links[index] ? `
        <a class="coach-v21-payment-card" data-coach-payment-formula="${type}" href="${links[index].href}" aria-label="Choisir ${label}, 90 euros">
          <span class="coach-v21-payment-photo" aria-hidden="true"></span>
          <span class="coach-v21-payment-copy"><small>Formule Découverte</small><strong>${label}</strong><em>${description}</em><b>90&nbsp;€</b></span>
