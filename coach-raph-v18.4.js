(() => {
  ...
  if (window.__coachRaphHomeV184) return;
  window.__coachRaphHomeV184 = true;
  ...
  function buildAfterCommercial(root, anchor) {
    let panel = root.querySelector(".coach-v184-after-commercial");
    ...
    panel = create("section", "coach-v184-after-commercial");
    panel.id = "coach-after";
    panel.innerHTML = `
      ...
      <div class="coach-v184-plan-grid">${followupPlans.map(([icon,title,price,copy], index) => `
        <article class="coach-v184-plan${index === 1 ? " is-featured" : ""}"><span class="coach-v184-plan-icon" aria-hidden="true">${icon}</span><div><small>${index ? "Avec Coach Raph" : "En autonomie"}</small><h3>${title}</h3><strong>${price}</strong><p>${copy}</p></div></article>`).join("")}</div>
      <div class="coach-v184-duo-card"><div class="coach-v184-duo-icon" aria-hidden="true">2</div><div><span>Formule Duo</span><h3>Commencer à deux</h3><p><strong>150 € pour deux personnes pendant 4 semaines</strong>, soit 75 € par personne.</p><ul><li>Deux bilans individuels</li><li>Un programme adapté à chacun</li><li>Des défis communs</li><li>Des conseils pour s’entraîner ensemble</li></ul></div>${duoLink ? `<a href="${duoLink.href}">Découvrir la formule Duo <span aria-hidden="true">→</span></a>` : ""}</div>`;
