(() => {
  ...
  function setupTestimonials(root) {
    const track = root.querySelector('.notion-column-list[data-coach-home-section="testimonials"]');
    if (!track || track.children.length < 2) return;
    track.classList.add("coach-v17-testimonials-track");
    track.querySelectorAll(":scope > .notion-column").forEach((card) => card.classList.add("coach-v17-testimonial"));
    if (sliders.has(track)) return;
