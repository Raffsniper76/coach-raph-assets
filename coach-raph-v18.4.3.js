(() => {
  ...
  if (window.__coachRaphHomeV1843) return;
  window.__coachRaphHomeV1843 = true;
  ...
  function buildPageEnding(root) {
    const existing = [...document.querySelectorAll("#coach-ending-v184, .coach-v184-ending")];
    const current = existing.find((candidate) => root.contains(candidate));
    existing.filter((candidate) => candidate !== current).forEach((duplicate) => duplicate.remove());

    const contactLink = [...root.querySelectorAll("a[href]")]
      .find((link) => /ecris-moi par e-mail/.test(normalize(link.textContent)));
    const legalNames = ["Mentions légales", "Conditions générales de vente", "Politique de confidentialité"];
    const legalLinks = legalNames.map((name) => [...root.querySelectorAll("a[href]")]
      .find((link) => normalize(link.textContent) === normalize(name))).filter(Boolean);
    if (!contactLink || legalLinks.length < 2) return current;

    const contactSource = contactLink.closest("[id^='block-']") || contactLink.closest(".notion-callout") || contactLink.parentElement;
    if (!contactSource) return current;

    let panel = current;
    if (!panel) {
      panel = create("section", "coach-v184-ending");
      panel.id = "coach-ending-v184";
      panel.dataset.coachGenerated = "page-ending";
      panel.innerHTML = `<div class="coach-v184-ending-contact"><span class="coach-v184-ending-icon" aria-hidden="true">?</span><div><small>Une question avant de réserver ?</small><strong>Parlons-en simplement.</strong></div><a href="${contactLink.href}">Écris-moi par e-mail <span aria-hidden="true">→</span></a></div><div class="coach-v184-ending-legal"><span>© 2023–2026 Coach Raph</span><nav aria-label="Informations légales">${legalLinks.map((link, index) => `<a href="${link.href}">${legalNames[index]}</a>`).join("")}</nav></div>`;
      contactSource.insertAdjacentElement("beforebegin", panel);
    }

    hideSource(contactSource, "coach-v184-ending-source");
    legalLinks.forEach((link) => {
      const item = link.closest("li") || link.closest("[id^='block-']") || link;
      hideSource(item, "coach-v184-ending-source");
      const list = item.closest("ul,ol,.notion-bulleted-list,.notion-numbered-list");
      if (list && [...list.querySelectorAll("a[href]")].every((candidate) => legalLinks.includes(candidate))) {
        hideSource(list, "coach-v184-ending-source");
      }
    });
    [...root.querySelectorAll("h1,h2,h3,p,.notion-heading,.notion-text,[id^='block-']")]
      .filter((node) => normalize(node.textContent) === "informations legales" && normalize(node.textContent).length < 40)
      .forEach((node) => hideSource(node, "coach-v184-ending-source"));
    return panel;
  }
